...
namespace Lockstep.UnsafeECS {
    ...
    public unsafe struct NativeArray<T> : IDisposable, IEnumerable<T>, IEquatable<NativeArray<T>>, IEnumerable
        ...
        private static unsafe void Allocate(int length, Allocator allocator, out NativeArray<T> array){
            ...
            long size = elemSize * (long) length;
            ...
            if (size > (long) int.MaxValue)
                throw new ArgumentOutOfRangeException(nameof(length),
                    string.Format("Length * sizeof(T) cannot exceed {0} bytes", (object) int.MaxValue));
            array.m_Buffer = UnsafeUtility.Malloc(size, elemSize, allocator);
            array.m_Length = length;
            array.m_AllocatorLabel = allocator;
            array.m_MinIndex = 0;
            array.m_MaxIndex = length - 1;
            array.m_ElemSize = (int) elemSize;
        }

        public unsafe void Realloc(int length, NativeArrayOptions options = NativeArrayOptions.ClearMemory){
            ...
            if (length <= this.m_Length) {
                throw new ArgumentException($"Realloc size{length} must bigger then the origin size{m_Length}");
            }

            long size = m_ElemSize * length;
            NativeArray<T>.IsBlittableAndThrow();
            if (size > (long) int.MaxValue)
                throw new ArgumentOutOfRangeException(nameof(length),
                    string.Format("Length * sizeof(T) cannot exceed {0} bytes", (object) int.MaxValue));
            var newBuffer = UnsafeUtility.Malloc(size, m_ElemSize, m_AllocatorLabel);
            UnsafeUtility.MemCpy(newBuffer, m_Buffer,   m_ElemSize * m_Length);
            ...
            if (options != NativeArrayOptions.ClearMemory)
                return;
            UnsafeUtility.MemClear(((byte*) this.m_Buffer) + rawLen * m_ElemSize,
                (long) ((length - rawLen) * m_ElemSize));
        }
        ...
        public ref T GetRef (in int index) {
            this.CheckElementReadAccess(index);
            return ref UnsafeUtility.ReadArrayElementRef<T>(this.m_Buffer, index);
        }
        ...
        public unsafe void Dispose(){
            ...
            if(m_Buffer == null) 
                throw new InvalidOperationException(
                    "The NativeArray can not be disposed twice!");
            ...
            if (index < this.Length && (this.m_MinIndex != 0 || this.m_MaxIndex != this.Length - 1))
                throw new IndexOutOfRangeException(
                    string.Format(
                        ...
                        (object) index, (object) this.m_MinIndex, (object) this.m_MaxIndex) +
                    "ReadWriteBuffers are restricted to only read & write the element at the job index. You can use double buffering strategies to avoid race conditions due to reading & writing in parallel to the same elements from a job.");
            throw new IndexOutOfRangeException(string.Format("Index {0} is out of range of '{1}' Length.",
                (object) index, (object) this.Length));
        }
        ...
        public struct Enumerator : IEnumerator<T>, IEnumerator, IDisposable {
            ...
            public T Current {
                get { return this.m_Array[this.m_Index]; }
            }
